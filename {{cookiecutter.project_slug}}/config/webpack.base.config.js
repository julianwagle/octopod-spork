...
module.exports = (opts) => {
  ...
  return {
    ...
    entry: {
      main: path.resolve(PROJECT_ROOT, '{{ cookiecutter.project_slug }}/static/index'),
      vendor: ['react', 'redux', 'react-router', 'react-redux', 'react-dom'],
    },

    output: {
      path: path.resolve(PROJECT_ROOT, '{{ cookiecutter.project_slug }}/static/bundles'),
    ...
    resolve: {
      extensions: ['.js', '.jsx'],
      modules: [
        path.resolve(PROJECT_ROOT, '{{ cookiecutter.project_slug }}/static'),
        'node_modules',
      ],
    },
  };
